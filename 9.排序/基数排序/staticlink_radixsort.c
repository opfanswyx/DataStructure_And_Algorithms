...
#define RADIX 10            //基数，例如关键字是数字，无疑由0~9 组成，基数就是10；如果关键字是字符串（字母组成），基数就是26
...
void Distribute(SLCell *r,int i,ArrType f,ArrType e){ 
    ...
    {
        int j=r[p].keys[i];                 //取出每个关键字的第i 位，由于采用的是最低位优先法，所以，例如，第1 位指的就是每个关键字的个位
        if (!f[j]) 
        {                                   //如果只想该位数字的指针不存在，说明这是第一个关键字，直接记录该关键字的位置即可
            f[j]=p;
        }
        else
        {                       //如果存在，说明之前已经有同该关键字相同位的记录，所以需要将其进行连接，将最后一个相同的关键字的next指针指向该关键字所在的位置，同时最后移动尾指针的位置。
            r[e[j]].next=p;
        }
        e[j]=p;                             //移动尾指针的位置
    }
...
void Collect(SLCell *r,int i,ArrType f,ArrType e)
{
    int j;
    int t;    
    for (j=0;!f[j]; j++);           //从0 开始遍历，查找头指针不为空的情况，为空表明该位没有该类型的关键字
...
void RadixSort(SLList *L)
{
    int i;
    ArrType f,e;  
    for (i=0; i<L->keynum; i++)     //根据记录中所包含的关键字的最大位数，一位一位的进行分配与收集
    {
        Distribute(L->r, i, f, e);      //秉着先分配后收集的顺序
        Collect(L->r, i, f, e);
    }
}
//创建静态链表
